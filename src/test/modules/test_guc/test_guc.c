 ...
 * test_guc.c
 *
 * Test module for GUC_EXTRA_IS_CONTEXT:
 * 1. Simple counter (without GUC_EXTRA_IS_CONTEXT flag - backward compatibility)
 * 2. Server pools (with GUC_EXTRA_IS_CONTEXT flag)
 */
 ...
 *
 * This demonstrates:
 * - List usage (server addresses)
 * - Nested structures (pool metadata)
 * - MemoryContext for complex data
 * - GucContextExtra wrapper pattern
 */
typedef struct ServerPool
{
	char	   *pool_name;
	List	   *servers;		/* List of server address strings */
	int			max_connections;
	int			timeout_seconds;
	char	   *description;	/* Generated summary */
}			ServerPool;
...
check_pool_guc(char **newval, void **extra, GucSource source)
{
	...
	*servers_part++ = '\0';
	...
	/* Create wrapper with context and data pointer */
	...
	*extra = wrapper;
	...
	elog(DEBUG1, "pool GUC: parsed pool '%s' with %d servers, context=%p, data=%p",
...
assign_pool_guc(const char *newval, void *extra)
{
	...
	elog(DEBUG1, "pool GUC: active pool '%s' with %d servers, context=%p, data=%p",
